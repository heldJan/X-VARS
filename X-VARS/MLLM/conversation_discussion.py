...
class Conversation:
    ...
    action_class = ""
    offence_class = ""
    prediction: str = " The prediction for this video is "
    ...
    def set_predictions(self, action_class, offence_class):
        ...
        print(self.action_class)
        print(self.offence_class)
            ...
            for role, message in self.messages:
                if message:
                    if type(message) is tuple:
                        message, _ = message
                    ret += role + ": " + message + self.prediction + self.sep
                else:
                    ret += role + ":"
            return ret
            ...
            for i, (role, message) in enumerate(self.messages):
                if message:
                    if type(message) is tuple:
                        ...
                        message = message_list[0] + self.prediction + self.action_class + self.offence_class + '\n<video>'
                        ret += role + ": " + message + seps[i % 2]
                    else:
                        if role == "USER":
                            ret += role + ": " + message + self.prediction + self.action_class + self.offence_class + seps[i % 2]
                        else:
                            ret += role + ": " + message + seps[i % 2]
                else:
                    ret += role + ":"
