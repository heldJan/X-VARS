...
def build_demo(embed_mode):
    ...
    with gr.Blocks(title="X-VARS", theme=seafoam, css=css) as demo:
        state = gr.State()
        img_list = gr.State()
        first_run = gr.State()
        ...
        with gr.Row():
            with gr.Column(scale=3):
                ...
                gr.Markdown(
                    ...
                    "before submitting question to X-VARS.",
                    style="color:gray")
            ...
            with gr.Column(scale=6):
                chatbot = grChatbot(elem_id="chatbot", label="X-VARS", visible=True).style(height=600)
                with gr.Row():
...
if __name__ == "__main__":
    ...
    #base_model = 'pretrained_video_chatgpt'
    base_model = 'MODEL_Train_for_3_epoch_FINAL'
    #path_to_llama_model = 'Train_for_3_epoch_FINAL'
    #path_to_projection_layer = 'video_chatgpt-7B.bin'
    path_to_projection_layer  = ''
    path_to_llama_model = ''

    model, vision_tower, tokenizer, image_processor, video_token_len = initialize_model(base_model, path_to_projection_layer)

    for name, param in model.state_dict().items():
        if name == "model.mm_projector.weight" or name == "model.mm_projector.base_layer.weight":
            print(name)
            print(param)


    vision_tower = MVNetwork().cuda()

    path_weights = "visual_encoder/14_model.pth.tar"
    load = torch.load(path_weights)
    vision_tower.load_state_dict(load['state_dict'], strict=False)


    if path_to_llama_model != '':
    ...
    chat = Chat(base_model, conv_mode, tokenizer, image_processor, vision_tower, model, replace_token)
